...
internal class ChatAttachmentDelegate(
    ...
    /**
     * Attaches a recorded voice message (.m4a file) via `file.attach`,
     * producing an audio attachment with a local file:// URI for playback.
     */
    fun attachAudioFile(state: MutableStateFlow<ChatUiState>, filePath: String) {
        val sessionId = state.value.activeSessionId ?: return
        if (state.value.isAttaching) return
        state.update { it.copy(isAttaching = true) }
        scope.launch(Dispatchers.IO) {
            try {
                ...
                if (!file.exists() || file.length() == 0L) {
                    throw IllegalStateException("Recording is empty")
                }
                val name = file.name
                ...
                val params = buildJsonObject {
                    put("session_id", sessionId)
                    put("data_url", "data:audio/mp4;base64,$b64")
                    put("name", name)
                }
                val result = gatewayClient.request("file.attach", jsonToElementMap(params))
                val ref = ((result as? JsonObject)?.get("ref_text") as? JsonPrimitive)?.content
                ...
                Timber.i("[Chat] Attached voice message $name (size=${bytes.size})")
            } catch (e: Exception) {
                Timber.e(e, "[Chat] Voice attach failed")
                state.update { it.copy(
                    errorEvent = ErrorEvent.Error("Voice attach failed: ${e.message}"),
                    isAttaching = false,
                ) }
            }
        }
