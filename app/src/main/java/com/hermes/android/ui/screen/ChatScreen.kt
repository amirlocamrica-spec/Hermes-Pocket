...
import com.hermes.android.audio.AudioRecorder
import com.hermes.android.audio.TtsPlayer
...
fun ChatScreen(
    ...
    val recordPermissionLauncher = rememberLauncherForActivityResult(
        ...
        if (granted) {
            // Starting a new recording: toggle was pressed while permission granted.
            if (!isRecording) {
                try {
                    val rec = AudioRecorder.create(context)
                    rec.start()
                    audioRecorder = rec
                    isRecording = true
                } catch (e: Exception) {
                    Toast.makeText(context, "Record start failed: ${e.message}", Toast.LENGTH_SHORT).show()
                }
            }
        } else {
    ...
    fun toggleRecording() {
        if (isRecording) {
            // Stop: finalize the recording and attach it like a file.
            val rec = audioRecorder
            audioRecorder = null
            isRecording = false
            if (rec != null) {
                val file = try { rec.stop() } catch (e: Exception) { null }
                if (file != null && file.exists() && file.length() > 0L) {
                    viewModel.attachAudioFile(file.absolutePath)
                } else {
                    Toast.makeText(context, "Recording was empty", Toast.LENGTH_SHORT).show()
                }
            }
        } else {
        ...
        Scaffold(
            ...
            Column(
                    ...
                    LazyColumn(
                        ...
                        itemsIndexed(
                            ...
                            MessageBubble(
                                ...
                                onSpeak = { text ->
                                    if (AppPrefs.isVoiceRepliesEnabled(context)) {
                                        ttsPlayer.initialize()
                                        ttsPlayer.speak(text, AppPrefs.getVoiceSpeed(context))
                                    }
                                },
                            )
                            }
                ...
                InputBar(
                    ...
                    onRecordVoiceToggle = { toggleRecording() },
